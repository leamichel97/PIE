# -*- coding: utf-8 -*-
"""
  mpa_base.py generated by WhatsOpt 1.8.2
"""
# DO NOT EDIT unless you know what you are doing
# whatsopt_url: 
# analysis_id: 459


import numpy as np
from numpy import nan
from os import path
from importlib import import_module
from yaml import load, FullLoader
from openmdao.api import ExplicitComponent

class MpaBase(ExplicitComponent):
    """ An OpenMDAO base component to encapsulate Mpa discipline """

    def __init__(self, **kwargs):
        super(MpaBase, self).__init__(**kwargs)
        self._impl = None
        dockconf = path.join(path.dirname(__file__), ".whatsopt_dock.yml")
        if path.exists(dockconf):
            with open(dockconf) as dockfile:
                dock = load(dockfile, Loader=FullLoader)
                impl = dock.get("mpa")
                if impl:
                    module = import_module(impl['module'])
                    self._impl = getattr(module, impl['class'])()

    def setup(self):
        self.add_input('MAIT', val=np.ones((32,)), desc='')
        self.add_input('M_PA_percentage', val=0.05, desc='')

        self.add_output('MPA', val=1.0, desc='')



        